"""
Construit un CSV d'annotations propre pour INbreast, compatible avec le pipeline
du projet (memes colonnes cles que breast-level_annotations.csv de VinDr-Mammo :
study_id, image_id, laterality, view_position, breast_density, split).

INbreast fournit un seul fichier INbreast.xls (colonne ACR = densite 1-4) et un
dossier ALL-IMGS de DICOM. Contrairement a VinDr/CBIS-DDSM :
  - Pas de split train/test officiel -> on en genere un, stratifie par densite,
    seed fixe pour rester reproductible.
  - La colonne "Patient ID" est anonymisee ("removed" dans ce fichier) -> on ne
    peut pas grouper par patiente ; chaque ligne = une image, donc study_id et
    image_id sont derives de l'identifiant de fichier (colonne "File Name").
  - "File Name" est stocke en flottant par Excel (ex. 22678622.0) ; il faut le
    reconvertir en entier puis retrouver le fichier .dcm reel dans ALL-IMGS, qui
    est nomme "{FileName}_{hash}_MG_{Side}_{ViewCode}_ANON.dcm".
  - Colonne ACR contient aussi des valeurs invalides (chaine vide, NaN, "28" en
    texte) -> filtrees par coercion numerique + isin({1,2,3,4}).
"""
import argparse
import glob
import os
...
def main():
    ...
    print(f"[INFO] {len(df)} lignes annotees valides -> {len(out_df)} images resolues sur disque, {not_found} non trouvees")
    ...
    print(f"[INFO] Repartition split:\n{out_df['split'].value_counts()}")
    print(f"[INFO] Repartition densite (total):\n{out_df['breast_density'].value_counts()}")
    print(f"[INFO] Repartition densite (train):\n{train_df['breast_density'].value_counts()}")
    print(f"[INFO] Repartition densite (test):\n{test_df['breast_density'].value_counts()}")
    print(f"[INFO] CSV sauvegarde: {args.out_csv}")
