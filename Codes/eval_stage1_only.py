"""
Evalue l'etage 1 ((A,B) vs (C,D)) de cascade_finetuned_pipeline.py isolement,
sur le test set complet -- meme logique que eval_stage2_only.py. Groupe A+B
contre C+D, moins desequilibre que A seule ou D seule, mais on verifie quand
meme le recall par groupe pour etre sur qu'il n'y a pas de biais cache.
"""
import argparse
import pandas as pd
import torch
...
def main():
    ...
    print(f"[INFO] {len(test_dataset)} images (A+B+C+D) dans le test set")
