...
CONFIG = {
    ...
    'EPOCHS': 10,
    'BACKBONE': 'resnet50',  # 'resnet50' ou 'rexnet_150'
    'BATCH_SIZE': 10,
    'LEARNING_RATE': 1e-4,
    ...
    'RESNET50_WEIGHTS_PATH': "featuresfinetuned_weights/resnet50_image_branch_best.pth",
...
class HybridModelManager:
    ...
    def create_model(self, input_channels=1, image_feature_dim=512,
                    ...
                    resnet50_weights=None, rexnet150_weights=None):
        """Crée le modèle hybride à deux branches (ResNet50/MLO et RexNet150/CC)."""
    ...
    def train_model(self, data_csv, image_root_cc, image_root_mlo, epochs=50, batch_size=8, lr=1e-4,
        ...
        # Création du modèle à deux branches
        ...
        # Geler les couches des branches images si demandé
        if freeze_image_branches:
            print("[INFO] Gel des couches des branches images (ResNet50 et RexNet150)")
            if hasattr(model, 'mlo_branch'):
                for param in model.mlo_branch.parameters():
                    param.requires_grad = False
            if hasattr(model, 'cc_branch'):
                for param in model.cc_branch.parameters():
                    param.requires_grad = False
...
def main():
    ...
    parser.add_argument('--backbone', default=CONFIG['BACKBONE'], choices=['resnet50', 'rexnet_150'], help='Backbone pour l\'image')
    ...
    try:
        ...
        trained_model = manager.train_model(
            ...
            # Le modèle utilise toujours les deux branches simultanément (MLO=ResNet50,
            # CC=RexNet150), donc leurs poids pré-fine-tunés doivent être chargés
            # inconditionnellement — --backbone ne sélectionne pas "l'unique" backbone,
            # c'était un reliquat qui empêchait une des deux branches de charger ses poids.
            resnet50_weights=CONFIG['RESNET50_WEIGHTS_PATH'],
            rexnet150_weights=CONFIG['REXNET150_WEIGHTS_PATH'],
