...
DEFAULT_WEIGHTS_PATH = "featuresfinetuned_weights/hybrid_model_best_resnet50_2branches.pth"
...
parser = argparse.ArgumentParser(description="Évaluation du modèle à double branche (RexNet150 / CC + ResNet50 / MLO)")
...
# les vues CC/MLO d'un même sein par (study_id, laterality). L'ancienne classe
# HybridTestDataset ne faisait AUCUN appariement réel : cc_image_id/mlo_image_id
# n'existent pas dans le CSV brut, donc row.get(...) retombait toujours sur
# row['image_id'] et les deux branches recevaient la même image dupliquée.
...
# Chemins des poids des branches images
...
print("=== Résultats sur le set de test ===")
print(report)
save_text_report(report, "hybrid_model_2branches_classification_report.txt")
...
plt.xlabel('Prédit')
plt.ylabel('Vrai')
plt.title('Matrice de confusion - Modèle hybride 2 branches (4 classes)')
save_plot(plt, "hybrid_model_2branches_confusion_matrix.png")