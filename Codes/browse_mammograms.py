...
class MammogramBrowser:
  ...
  def __init__(self, annotations_csv, image_root):
    ...
    print(f"Base de données chargée: {len(self.df)} images")
    print(f"Répartition par densité:")
  ...
  def get_random_sample(self, num_images=10, density_filter=None, split_filter=None):
    ...
    if density_filter:
      ...
      print(f"Filtrage par densité: {density_filter}")
    
    if split_filter:
      filtered_df = filtered_df[filtered_df['split'] == split_filter]
      print(f"Filtrage par split: {split_filter}")
    
    if len(filtered_df) == 0:
      print(f"Aucune image trouvée avec les filtres spécifiés")
      return pd.DataFrame()
    ...
    print(f"Échantillon de {len(sample_df)} images sélectionné")
    return sample_df
