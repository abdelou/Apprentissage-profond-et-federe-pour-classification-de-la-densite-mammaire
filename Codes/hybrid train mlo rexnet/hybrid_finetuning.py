...
class FocalLoss(nn.Module):
  """Focal Loss (Lin et al. 2017), pour mieux traiter la classe A tres
  minoritaire. Contrairement a la ponderation de classe simple (deja
  utilisee ailleurs dans le projet et insuffisante sur cette classe,
  voir Section test-seuil-a du memoire), la Focal Loss reduit
  dynamiquement le poids des exemples deja bien classes (faciles),
  quelle que soit leur classe, pour concentrer l'apprentissage sur les
  exemples difficiles -- notamment les rares exemples natifs de A."""
...
class HybridModelManager:
  ...
  def _train_model(self, model, train_loader, val_loader, epochs=50, lr=1e-4,
    ...
    model.to(self.device)
    if use_focal_loss:
      print(f"[ENTRAÎNEMENT] Utilisation de la Focal Loss (gamma={focal_gamma})")
    ...
    for epoch in range(epochs):
      ...
      for batch_idx, (images, hists, labels) in enumerate(progress_bar):
        ...
        # Extraction directe des features GLCM (sans MLP)
        ...
        glcm_features = torch.tensor(glcm_features, dtype=torch.float32, device=images.device)
        logits_global = model.classifier(torch.cat([model.image_branch(images), glcm_features], dim=1))
        ...
        loss = criterion(logits_global, labels) # Use only the global logits for loss calculation
        loss.backward()
        optimizer.step()
      ...
      train_true = []
      train_pred = []
      for images, hists, labels in train_loader:
        images, hists, labels = images.to(self.device), hists.to(self.device), labels.to(self.device)
        batch_size = images.shape[0]
        glcm_features = []
        for i in range(batch_size):
    ...
    print(f"\n[FIN] Entraînement terminé du modèle hybride avec rexnet150 sur MLO ")
...
def main():
  ...
  parser.add_argument('--use_focal_loss', action='store_true', help='Utiliser la Focal Loss au lieu de CrossEntropyLoss (mieux pour la classe A minoritaire)')
  parser.add_argument('--focal_gamma', type=float, default=2.0, help='Parametre gamma de la Focal Loss')
