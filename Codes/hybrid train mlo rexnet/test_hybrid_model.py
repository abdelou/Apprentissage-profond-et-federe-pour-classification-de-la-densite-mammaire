...
search_dirs = [
    os.path.join(script_dir, "featuresfinetuned_weights"),
    "/home_nfs/abdelouahada/Codes_cetic/Codes/hybrid train mlo rexnet/featuresfinetuned_weights",
    "/home_nfs/abdelouahada/Codes_cetic/Codes/hybrid train MLO rexnet/featuresfinetuned_weights",
    "featuresfinetuned_weights"
]

DEFAULT_WEIGHTS_PATH = None
for d in search_dirs:
    if os.path.exists(d):
        pth_files = [os.path.join(d, f) for f in os.listdir(d) if f.endswith('.pth')]
        best_pth = [f for f in pth_files if 'best' in f]
        if best_pth:
            DEFAULT_WEIGHTS_PATH = best_pth[0]
            break
        elif pth_files:
            DEFAULT_WEIGHTS_PATH = pth_files[0]
            break

if not DEFAULT_WEIGHTS_PATH:
    DEFAULT_WEIGHTS_PATH = "featuresfinetuned_weights/hybrid_model_best_cnn.pth"

print(f"[INFO] Poids résolus dynamiquement à : {DEFAULT_WEIGHTS_PATH}")
...
parser = argparse.ArgumentParser(description="Évaluation du modèle hybride RexNet150 + Histogramme")
parser.add_argument('--weights_path', default=DEFAULT_WEIGHTS_PATH, help="Chemin vers les poids du modèle")
parser.add_argument('--annotations_csv', default=DEFAULT_ANNOTATIONS_CSV, help="Chemin vers le CSV d'annotations")
parser.add_argument('--image_root', default=DEFAULT_IMAGE_ROOT, help="Chemin vers le dossier des images")
...
# Le modèle est entraîné uniquement sur les vues MLO (hybrid_finetuning.py filtre
# df["view_position"] == "MLO"), donc le test doit utiliser le même filtre pour rester
# cohérent avec le Val Acc obtenu à l'entraînement et comparable aux autres approches.
if "view_position" in df_test.columns:
    df_test = df_test[df_test["view_position"] == "MLO"].reset_index(drop=True)
print(f"[DEBUG] Lignes CSV split=test (vue MLO): {len(df_test)}")
...
print(f"[DEBUG] Nombre de batches attendus: {len(test_loader)} (batch_size={BATCH_SIZE})")
...
plt.xlabel('Prédit')
plt.ylabel('Vrai')
plt.title('Matrice de confusion - Modèle hybride (4 classes)')
