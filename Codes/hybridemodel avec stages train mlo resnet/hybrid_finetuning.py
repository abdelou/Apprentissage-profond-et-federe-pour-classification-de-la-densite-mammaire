...
CONFIG = {
    'ANNOTATIONS_CSV': 'DDSM/output_annotations_MLO.csv',
    # TODO: pointe vers ton propre export DDSM (vues MLO) sur le cluster - à adapter.
    ...
    'FINETUNED_WEIGHTS_DIR': 'model3stagesMLO',
    'RESUME_FROM': 'model3stagesMLO/hybrid_model_checkpoint_cnn.pth',
    'FINETUNED_WEIGHTS_PATH': 'model3stagesMLO/cnn_image_branch_best.pth'
}
    ...
    def _train_model(self, model, train_loader, val_loader, epochs=50, lr=1e-4,
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (images, hists, labels) in enumerate(progress_bar):
                ...
                optimizer.zero_grad()
                # Forward pipeline à deux étages
                # Logits globaux (classifieur 4 classes)
                logits_global = model.classifier_global(model.norm_concat(torch.cat([model.image_branch(images), model.histogram_branch(hists)], dim=1)))
                # Logits hiérarchiques (pipeline à deux étages)
                logits = logits_global  # Pour la compatibilité, mais on utilise logits_global pour la prédiction finale

                # Stage 1: AB vs CD
                abcd_targets = (labels < 2).long()  # 0 if A/B, 1 if C/D
                ...
                if ab_mask.any():
                    ...
                    valid_ab = (ab_targets == 0) | (ab_targets == 1)
                    if valid_ab.any():
                        ab_logits = model.classifier_ab(model.norm_concat(torch.cat([
                            model.image_branch(images[ab_mask][valid_ab]),
                            model.histogram_branch(hists[ab_mask][valid_ab])
                        ], dim=1)))
                        ab_targets = ab_targets[valid_ab]
                        loss_stage2 += criterion(ab_logits, ab_targets)
                if cd_mask.any():
                    cd_targets = labels[cd_mask]
                    valid_cd = (cd_targets == 2) | (cd_targets == 3)
                    if valid_cd.any():
                        cd_logits = model.classifier_cd(model.norm_concat(torch.cat([
                            model.image_branch(images[cd_mask][valid_cd]),
                            model.histogram_branch(hists[cd_mask][valid_cd])
                        ], dim=1)))
                        cd_targets = cd_targets[valid_cd] - 2  # C=0, D=1
                        loss_stage2 += criterion(cd_logits, cd_targets)
                ...
                _, predicted = logits_global.max(1)
            ...
            with torch.no_grad():
                for images, hists, labels in val_loader:
                    images, hists, labels = images.to(self.device), hists.to(self.device), labels.to(self.device)
                    # Prédiction finale et loss avec le classifieur global
                    logits_global = model.classifier_global(model.norm_concat(torch.cat([model.image_branch(images), model.histogram_branch(hists)], dim=1)))
                    pred_final = logits_global.argmax(dim=1)
                    val_loss += criterion(logits_global, labels).item()
                    val_total += labels.size(0)
                    val_correct += pred_final.eq(labels).sum().item()
                    all_true.extend(labels.cpu().numpy())
                    all_pred_final.extend(pred_final.cpu().numpy())

                    # Pour analyse hiérarchique uniquement (matrices de confusion)
                    stage1_logits = model.classifier_stage1(model.norm_concat(torch.cat([model.image_branch(images), model.histogram_branch(hists)], dim=1)))
                    pred_stage1 = stage1_logits.argmax(dim=1)
                    all_pred_stage1.extend(pred_stage1.cpu().numpy())

                    ab_mask = (pred_stage1 == 0)
                    cd_mask = (pred_stage1 == 1)
                    if ab_mask.any():
                        ab_logits = model.classifier_ab(model.norm_concat(torch.cat([model.image_branch(images[ab_mask]), model.histogram_branch(hists[ab_mask])], dim=1)))
                        pred_ab = ab_logits.argmax(dim=1)
                        all_pred_ab.extend(pred_ab.cpu().numpy())
                    if cd_mask.any():
                        cd_logits = model.classifier_cd(model.norm_concat(torch.cat([model.image_branch(images[cd_mask]), model.histogram_branch(hists[cd_mask])], dim=1)))
                        pred_cd = cd_logits.argmax(dim=1)
                        all_pred_cd.extend(pred_cd.cpu().numpy())
            ...
            print("\nMatrice de confusion AB vs CD (stage 1):")
            print(confusion_matrix((np.array(all_true) < 2).astype(int), all_pred_stage1))

            # Pour A vs B, ne garder que les labels et prédictions où stage1_pred == 0
            ab_mask = np.array(all_pred_stage1) == 0
            ab_true = np.array(all_true)[ab_mask]
            ab_pred = np.array(all_pred_ab)
            if len(ab_true) == len(ab_pred) and len(ab_true) > 0:
                print("\nMatrice de confusion A vs B (stage 2):")
                print(confusion_matrix(ab_true, ab_pred))
            else:
                print(f"\nImpossible de calculer la matrice A vs B: tailles incohérentes ({len(ab_true)} vs {len(ab_pred)})")

            # Pour C vs D, ne garder que les labels et prédictions où stage1_pred == 1
            cd_mask = np.array(all_pred_stage1) == 1
            cd_true = np.array(all_true)[cd_mask] - 2
            cd_pred = np.array(all_pred_cd)
            if len(cd_true) == len(cd_pred) and len(cd_true) > 0:
                print("\nMatrice de confusion C vs D (stage 2):")
                print(confusion_matrix(cd_true, cd_pred))
            else:
                print(f"\nImpossible de calculer la matrice C vs D: tailles incohérentes ({len(cd_true)} vs {len(cd_pred)})")
        ...
        print(f"\n[FIN] Entraînement terminé du modèle hybride avec resnet50 sur MLO ")
...
def main():
    ...
    parser.add_argument('--finetuned_weights_path', default=CONFIG['FINETUNED_WEIGHTS_PATH'], help='Chemin vers les poids fine-tunés de la branche image')
