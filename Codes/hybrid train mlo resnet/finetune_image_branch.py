...
CONFIG = {
    ...
    'RESUME_FROM': 'featuresfinetuned_weights/cnn_image_branch_checkpoint_resnet50.pth'
}
    ...
    def _train_model(self, model, train_loader, val_loader, epochs=30, lr=1e-4,
        ...
        for epoch in range(epochs):
            ...
            if val_acc > best_val_acc + min_delta:
                ...
                best_model_path = os.path.join(save_dir, f'{backbone_name}_image_branch_best_resnet50.pth')
                model.save_finetuned_weights(best_model_path)
                
                # Sauvegarde du checkpoint pour reprise
                checkpoint_path = os.path.join(save_dir, f'{backbone_name}_image_branch_checkpoint_resnet50.pth')
                torch.save({
        ...
        # Sauvegarde finale
        backbone_name = model.backbone
        final_save_path = os.path.join(save_dir, f'{backbone_name}_image_branch_final_resnet50.pth')
        ...
        print(f"  - Meilleur modèle: {os.path.join(save_dir, f'{backbone_name}_image_branch_best_resnet50.pth')}")
