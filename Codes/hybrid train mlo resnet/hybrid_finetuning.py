...
class HybridMammographyDataset(Dataset):
    """
    Dataset unifié pour charger les mammographies avec support pour data augmentation et histogrammes 256 bins.
    """
...
class HybridModelManager:
    """
    Gestionnaire pour l'entraînement et l'évaluation du modèle hybride sur vues MLO.
    """
    ...
    def create_model(self, input_channels=1, image_feature_dim=512, hist_hidden_dims=[128, 64],
        ...
        """Crée le modèle hybride (Section 6.2.4.1 ResNet50 sur MLO)."""
    ...
    def train_model(self, data_csv, image_root, epochs=5, batch_size=8, lr=1e-4,
        ...
        """
        Entraîne le modèle hybride ResNet50 sur MLO.
        """
        if save_dir is None:
        ...
        # Ensemble de validation à partir du train uniquement
        ...
        print(f"Distribution des données (Vues MLO):")
    ...
    def _train_model(self, model, train_loader, val_loader, epochs=5, lr=1e-4,
        ...
        """
        Entraînement du modèle hybride ResNet50 sur vues MLO.
        """
        model.to(self.device)
        ...
        print(f"[ENTRAÎNEMENT] Début du fine-tuning ResNet50 sur MLO pour {epochs} époques")
        ...
        print(f"\n[FIN] Entraînement terminé du modèle hybride avec ResNet50 sur MLO (Figure 15)")
...
def main():
    parser = argparse.ArgumentParser(description='Entraînement ResNet50 sur MLO (Figure 15)')
    ...
    parser.add_argument('--finetuned_weights_path', default=None, help='Poids fine-tunés')
    ...
    print(f"=== Entraînement ResNet50 sur MLO (Figure 15) ===")
    ...
    try:
        ...
        print("=== Entraînement réussi (Figure 15 ResNet50 MLO) ===")
    except Exception as e:
