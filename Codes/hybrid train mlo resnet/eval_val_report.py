...
view_position = sys.argv[1] if len(sys.argv) > 1 else 'MLO'
backbone = sys.argv[2] if len(sys.argv) > 2 else 'cnn'
weights_path = sys.argv[3] if len(sys.argv) > 3 else f"featuresfinetuned_weights/hybrid_model_best_{view_position.lower()}_{backbone}_GLCM.pth"
