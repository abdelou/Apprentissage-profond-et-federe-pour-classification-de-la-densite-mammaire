"""
Lance l'analyse d'interpretabilite (t-SNE + Grad-CAM) sur le modele final
retenu pour le memoire : Approche 3, ResNet50 + Histogramme (vues MLO).

Ce modele prend deux entrees (image + histogramme), alors que les fonctions
generiques de interpretability_analysis.py n'attendent qu'une image en
entree -- on enveloppe donc le modele dans un petit wrapper qui calcule
l'histogramme a la volee a partir de l'image, pour presenter une interface
a une seule entree a Grad-CAM/t-SNE. La cible Grad-CAM reste la derniere
couche convolutive de la branche image (ResNet50 layer4) : c'est elle qui
determine la prediction visuelle, la branche histogramme n'a pas de
structure spatiale a visualiser.
"""
import sys
import os
...
def main():
    ...
    print(f"[INFO] {len(dataset)} images de test, {len(dataset_train)} images de train chargees")
