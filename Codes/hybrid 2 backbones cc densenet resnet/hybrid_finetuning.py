...
from hybrid_model import DualBackboneClassifier
...
class SingleViewDataset(Dataset):
    def __init__(self, df, image_dir, view_position=VIEW, use_augmentation=False, split='training'):
        ...
        if use_augmentation and split == 'training':
            ...
            target_count = int(class_counts.max() * 0.5)
            parts = []
            for d_name in DENSITY_CLASSES:
                c_df = self.df[self.df['breast_density'] == d_name]
                if len(c_df) == 0:
                    continue
                if len(c_df) < target_count:
                    mult = int(np.ceil(target_count / len(c_df)))
                    parts.append(pd.concat([c_df] * mult, ignore_index=True).iloc[:target_count])
                else:
                    parts.append(c_df)
            self.df = pd.concat(parts, ignore_index=True).sample(frac=1.0, random_state=42).reset_index(drop=True)
...
def train_and_eval(view=VIEW, backbone_a='densenet121', backbone_b='resnet50', epochs=10, batch_size=8, lr=1e-4):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n=======================================================")
    print(f"ENTRAÎNEMENT MODÈLE DOUBLE-BACKBONE : {backbone_a.upper()} + {backbone_b.upper()} (vue {view})")
    print(f"=======================================================")
    ...
    train_dataset = SingleViewDataset(train_df[train_df['temp_split'] == 'training'], VINDR_ROOT, view, use_augmentation=True, split='training')
    val_dataset = SingleViewDataset(train_df[train_df['temp_split'] == 'validation'], VINDR_ROOT, view, use_augmentation=False, split='training')
    test_dataset = SingleViewDataset(test_df, VINDR_ROOT, view, use_augmentation=False, split='test')
    ...
    best_weights_path = f"featuresfinetuned_weights/dualbackbone_best_{tag}.pth"
    history_path = f"featuresfinetuned_weights/history_{tag}.json"
    os.makedirs("featuresfinetuned_weights", exist_ok=True)
    history = {"epochs": [], "train_acc": [], "val_acc": []}
    best_val_acc = 0.0
    ...
    if os.path.exists(best_weights_path):
        ...
        for epoch in range(epochs):
            ...
            for images, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} [{tag}]"):
            ...
            if val_acc > best_val_acc:
                ...
                model.save_finetuned_weights(best_weights_path)
                print(f"✅ Meilleur modèle sauvegardé: {best_weights_path} ({val_acc*100:.2f}%)")

            scheduler.step()

    print(f"\n=======================================================")
    print(f"RÉSULTAT DU TEST FINAL ({backbone_a.upper()} + {backbone_b.upper()}, vue {view})")
    print(f"=======================================================")
...
if __name__ == '__main__':
    ...
    train_and_eval(view=view, epochs=epochs)
