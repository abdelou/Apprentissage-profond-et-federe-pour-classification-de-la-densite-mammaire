...
CONFIG = {
    ...
    'RESUME_FROM': 'featuresfinetuned_weights/cnn_image_branch_checkpoint_rexnet150.pth'
}
    ...
    def __init__(self, annotations_df, image_dir, classes_to_use=None, label_map=None, 
        ...
        if self.use_augmentation and split == 'training':
            self.augmentation_transforms = transforms.Compose([
                transforms.RandomRotation(degrees=3),  # Rotation légère
...
class ImageBranchManager:
    ...
    def create_model(self, input_channels=1, feature_dim=4, pretrained=True):
        """Crée le modèle de branche image."""
    ...
    def finetune_model(self, data_csv, image_root, epochs=30, batch_size=8, lr=1e-4,
                      save_dir=None, patience=10, min_delta=0.0005,
    ...
    def evaluate_on_test(self, model, image_root, batch_size=8):
        ...
        test_dataset = ImageOnlyDataset(
            ...
            image_root,
            label_map=CONFIG['CLASS_MAP'],
            use_augmentation=False,  # Pas d'augmentation sur le test
            split='test',
        )
    ...
    def _train_model(self, model, train_loader, val_loader, epochs=30, lr=1e-4,
        ...
        print(f"[FINETUNE] Début du fine-tuning de la branche image pour {epochs} époques")
        print(f"[FINETUNE] Learning rate: {lr}")
        ...
        for epoch in range(epochs):
            ...
            if val_acc > best_val_acc + min_delta:
                ...
                best_model_path = os.path.join(save_dir, f'{backbone_name}_image_branch_best_rexnet150.pth')
                model.save_finetuned_weights(best_model_path)
                
                # Sauvegarde du checkpoint pour reprise
                checkpoint_path = os.path.join(save_dir, f'{backbone_name}_image_branch_checkpoint_rexnet150.pth')
                torch.save({
        ...
        # Sauvegarde finale
        backbone_name = model.backbone
        final_save_path = os.path.join(save_dir, f'{backbone_name}_image_branch_final_rexnet150.pth')
        ...
        print(f"  - Meilleur modèle: {os.path.join(save_dir, f'{backbone_name}_image_branch_best_rexnet150.pth')}")
...
def main():
    ...
    parser.add_argument('--resume_from',default=CONFIG['RESUME_FROM'], help='Chemin vers un checkpoint pour reprendre l\'entraînement')
    parser.add_argument('--eval_only', action='store_true', help='Évaluer uniquement le modèle (pas d\'entraînement)')
    parser.add_argument('--model_path', default=CONFIG['RESUME_FROM'], help='Chemin vers le modèle à évaluer')
