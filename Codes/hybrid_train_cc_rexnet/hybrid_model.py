...
class GLCMDescriptorMLP(nn.Module):
  ...
  def __init__(self, hidden_dims = [32, 16], dropout = 0.2, num_classes = None):
    super().__init__()
  ...
  def forward(self, image_tensor, classify = False):
  ...
  def save_finetuned_weights(self, save_path, include_classifier = False):
  ...
  def load_finetuned_weights(self, load_path, load_classifier = False):
...
class HybridMammographyClassifier(nn.Module):
  ...
  def forward(self, image_tensor, hist_tensor = None) :
    """
