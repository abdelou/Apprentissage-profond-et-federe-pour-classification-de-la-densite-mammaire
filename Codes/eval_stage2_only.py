"""
Evalue l'etage 2 (A vs B) de cascade_finetuned_pipeline.py isolement, sur le
test set complet (pas seulement le split de validation interne issu du
train), pour voir la matrice de confusion par classe -- l'accuracy globale
(98.76% en validation) ne dit pas si le modele est bon sur A et B separement.
"""
import argparse
import pandas as pd
import torch
...
def main():
    ...
    print(f"[INFO] {len(test_dataset)} images A/B dans le test set")
