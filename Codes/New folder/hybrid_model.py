...
class HistogramMLP(nn.Module):
  """
  Branche MLP pour traiter l'histogramme de niveaux de gris.
  
  Args:
    input_dim: Dimension de l'histogramme (256 par défaut)
    hidden_dims: Liste des dimensions cachées
    dropout: Taux de dropout
  """
  
  def __init__(self, input_dim = 256, hidden_dims = [128, 64], dropout = 0.2):
    super(HistogramMLP, self).__init__()
    ...
    self.histogram_mlp = nn.Sequential(*layers)
    self.output_dim = hidden_dims[-1]
  
  def forward(self, hist_tensor) :
    """
    Forward pass pour l'histogramme.
    
    Args:
      hist_tensor: Tenseur d'histogramme de forme (batch_size, 256)
      
    Returns:
      Features extraites de l'histogramme
    """
    return self.histogram_mlp(hist_tensor)
...
class ImageBranch(nn.Module):
  ...
  def __init__(self, backbone = 'cnn', input_channels = 1, feature_dim = 512, 
    ...
    if backbone == 'cnn':
      print(f"[ImageBranch] construction de la branche image avec CNN")
      time.sleep(5)
      ...
      try:
        ...
        # Nous utilisons maintenant la même taille de patch que le modèle pré-entraîné,
        # donc pas besoin d'ajuster les position embeddings
  ...
  def load_finetuned_weights(self, load_path, device = 'cpu', freeze_layers = True):
    """
...
class HybridMammographyClassifier(nn.Module):
  ...
  def __init__(self, 
    ...
    # Branche histogramme
    self.histogram_branch = HistogramMLP(
      input_dim=256,
      hidden_dims=hist_hidden_dims,
      dropout=dropout
    )
    
    # Déterminer la dimension des features d'image en fonction du backbone
    if backbone == 'vit':
      self.image_feature_dim = 768 # Dimension fixe pour ViT
    else:
      self.image_feature_dim = image_feature_dim # Dimension configurable pour CNN

    # Calcul de la dimension totale après concaténation
  ...
  def forward(self, image_tensor, hist_tensor) :
    ...
    # Extraction des features d'histogramme
