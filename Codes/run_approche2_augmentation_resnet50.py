#!/usr/bin/env python3
"""
Script d'exécution complet pour l'approche 2 (hiérarchique) avec données augmentées
et extracteur ResNet50. Il génère le CSV augmenté, extrait les features (ResNet50) puis
entraîne/évalue la pipeline 3-étapes.

Usage exemple:
    python run_approche2_augmentation_resnet50.py \
        --annotations_csv DDSM/annotations.csv \
        --image_root /path/to/images \
        --out_aug_csv DDSM/annotations_augmented_target4000.csv
...
from generate_augmented_dataset import main as gen_aug_main  # on pourra réutiliser la logique
...
def generate_augmented_csv(annotations_csv, image_root, out_csv):
    # Appelle le script de génération (fonction main) via subprocess serait propre, mais
    # pour garder simple on importe DataAugmentationManager comme dans generate_augmented_dataset
    from generate_augmented_dataset import main as gen_main
    # Construire les arguments pour le script générateur
    # Ici on invoque le script en mode CLI en appel direct n'est pas trivial :
    # pour simplicité, on réutilise generate_augmented_dataset.py via os.system
    cmd = f"python3 generate_augmented_dataset.py --annotations_csv {annotations_csv} --image_root {image_root} --out_csv {out_csv}"
    print('[run_resnet] Génération CSV augmenté: ' + cmd)
    ret = os.system(cmd)
    if ret != 0:
...
def extract_features_from_csv(backbone, augmented_csv, image_root, device, out_features_prefix):
    ...
    # Créer dossier
    os.makedirs(os.path.dirname(out_features_prefix), exist_ok=True)
